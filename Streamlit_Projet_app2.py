...
@st.cache_data
def load_data():
    North_hemisphere = pd.read_csv('data/NH.Ts+dSST.csv', header = 1, na_values = ['***'])
    South_hemisphere = pd.read_csv('data/SH.Ts+dSST.csv', header = 1, na_values = ['***'])
    Global=pd.read_csv('data/GLB.Ts+dSST.csv', header = 1, na_values = ['***'])
    Zone=pd.read_csv('data/ZonAnn.Ts+dSST.csv', index_col = 'Year')
...
set_global_style()
#  Image de bandeau
image_path= r'images/bandeau streamlit_2.png'
...
image_path = r'images/planète.jpg'
image_base64 = get_base64_im(image_path)
with st.sidebar:
   st.markdown(
    f"""
   ...
   image_path = r'images/datascientest.jpg'
   image_base64 = get_base64_im(image_path)
   st.markdown(f"""
...
if page == pages[0] : 
    ...
    if option_df == 'NASA: Datasets par hémisphère et Global':
        ...
        with col1:
         image = Image.open(r'images/nasa.png')  
...
""")
        ...
        with col7:
          image = Image.open(r'images/OWID.png')  
          st.image(image, width=100) 
        with col8:
...
##### RECHAUFFEMENT CLIMATIQUE PAR ZONES GEOGRAPHIQUES ######
  ...
  # GRAPHIQUE 9 : CARTE EVOLUTION TEMPERATURE PAR ZONE GEO
    with tab3:
      image = Image.open("images/Capture_cartopy.jpg")
      st.image(image, caption="Évolution de la température par zone (1880 → 2024)", use_container_width=True)
...
#Application de la régression linéaire
  ...
  #Décision Tree Regressor
    with st.expander("🌳 Decision Tree Regressor", expanded=False):
        model_tree = joblib.load(r"models/model_tree.pkl")
        col1, col2 = st.columns(2)
  ...
  #Random Forest Regressor
    with st.expander("🌲 Random Forest Regressor", expanded=False):
        rfr = joblib.load(r"models/rfr_compressed.pkl")
        col1, col2 = st.columns(2)
...
# jeu de test 
    ...
    with st.expander("BONUS 🌲 Random Forest Regressor avec normalisation des données", expanded=False):    
       clf = joblib.load(r"models/clf_compressed.pkl")
    ...
    model_fit = joblib.load(r"models/model_arima.pkl")
